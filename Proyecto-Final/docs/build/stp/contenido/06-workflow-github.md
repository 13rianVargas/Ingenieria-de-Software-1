...
```
Backlog → Por ejecutar → En ejecución → Pass / Fail → Cerrado
```
...
3. **En ejecución:** El analista esta ejecutando el TC en ese momento.
...
| Etiqueta | Valores | Propósito |
|---|---|---|
| Tipo | test-case, bug | Distinguir naturaleza del Issue |
| HU | HU-01 | Asociar al backlog |
| Prioridad | priority:alta, priority:media, priority:baja | Orden de ejecución / correccion |
...
Se mantiene un indice navegable en el repositorio (a definir su ubicacion exacta, pero se sugiere `Proyecto-Final/docs/casos-de-prueba/indice.md` cuando se ejecute la fase 9 del plan). El indice tiene una tabla con: ID, descripción corta, prioridad, estado actual, link al Issue de GitHub.
...
| Analista de Pruebas | Diseño y ejecución de casos, reporte de defectos | Avila Cortes Julian David, Vargas Clavijo Brian Steven |
...
3. **Ejecución:** el analista cambia el estado a "En ejecución", ejecuta los pasos, captura evidencias.
...
5. Asignar al dev responsable del módulo afectado segun ownership.
