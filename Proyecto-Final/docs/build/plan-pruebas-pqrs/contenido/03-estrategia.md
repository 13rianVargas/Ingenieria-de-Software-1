...
No se realizan pruebas unitarias, de integracion tecnica ni no funcionales en este plan. Esas categorias son responsabilidad de cada dev (Karma+Jasmine para frontend, JUnit+Spring Boot Test para backend) y se cubren en el pipeline CI del repositorio.

## Tecnicas de Diseño de Casos

| Tecnica | Aplicacion en HU-01 |
|---|---|
| Particion de equivalencias | Clasificar entradas validas e invalidas por campo: tipo PQRS, asunto, descripcion, formato del adjunto. |
...
| Tabla de decisiones | Combinacion cliente autenticado vs anonimo, con/sin adjunto, SMTP disponible/caido. |
...
Los casos de prueba se gestionan en **GitHub Issues** del repositorio del proyecto usando la plantilla `test-case.yml`. Los defectos se reportan con la plantilla `bug-report.yml`. La estructura completa de etiquetas, hitos, tablero de seguimiento y flujo de ejecucion se detalla en la seccion Workflow GitHub.
...
| GitHub Actions | Validaciones automaticas del repositorio (CI) — no ejecuta los TC funcionales, pero si los unit tests del software |
